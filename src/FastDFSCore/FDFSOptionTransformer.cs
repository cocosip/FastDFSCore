...
namespace FastDFSCore
{
    /// <summary>配置信息转换器
    /// </summary>
    public class FDFSOptionTransformer : IFDFSOptionTransformer
    {
        /// <summary>从文件中读取配置信息
        /// </summary>
        public FDFSOption GetOptionFromFile(string file)
        {
            if (!File.Exists(file))
            {
            ...
            using (var reader = XmlReader.Create(file, settings))
            {
                ...
                //charset
                option.Charset = root.SelectSingleNode("Charset").InnerText;
                //ConnectionTimeout
        ...
        /// </summary>
        public string ToXml(FDFSOption option)
        {
            try
            {
